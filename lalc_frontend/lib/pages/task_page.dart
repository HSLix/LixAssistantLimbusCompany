...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  String _taskName(BuildContext ctx, String key) {
    final s = S.of(ctx);
    switch (key) {
      case 'Daily Lunacy Purchase':
        return s.daily_lunacy_purchase;
      case 'Mail':
        return s.mail;
      case 'E.G.O':
        return 'E.G.O';
  ...
  List<bool> taskEnabled = [true, true, false, true, true, true, true, true];

  /* ===== 真正检查：已选队伍但"首选饰品流派"为空 ===== */

  // 仅 EXP / Thread / Mirror 有次数
  Map<String, int> taskCounts = {'EXP': 1, 'Thread': 1, 'Mirror': 1};

  // 各任务详细配置
  Map<String, dynamic> taskConfigs = {
    'Daily Lunacy Purchase': 0,
    'At Last': 'Do Nothing',
    'EXP': {'luxcavationMode': 'Enter', 'expStage': '09'},
    'Thread': {'luxcavationMode': 'Enter', 'threadStage': '60'},
    'Mirror': {
  ...
  Map<String, List<int>> taskTeams = {'EXP': [], 'Thread': [], 'Mirror': []};
  ...
  ''';
        ...
        throw Exception(
          'Failed to load release info from backup: ${response.statusCode}',
        );
      }
    } catch (e) {
      ...
      if (configManager.taskConfigs.containsKey(taskName)) {
        ...
        if (taskName == 'Daily Lunacy Purchase') {
          ...
          taskConfigs[taskName] =
              configManager.taskConfigs[taskName]!.params['value'] ??
              'Do Nothing';
        } else {
          taskConfigs[taskName] = Map<String, dynamic>.from(
            configManager.taskConfigs[taskName]!.params,
          );
          // 为镜像迷宫任务初始化mirror_mode参数，如果不存在的话
          if (taskName == 'Mirror' &&
              !taskConfigs[taskName].containsKey('mirror_mode')) {
            taskConfigs[taskName]['mirror_mode'] = 'normal'; // 默认为普通难度（单值）
          }

          // 为镜像迷宫任务初始化accept_reward参数，如果不存在的话
          if (taskName == 'Mirror' &&
              !taskConfigs[taskName].containsKey('accept_reward')) {
            taskConfigs[taskName]['accept_reward'] = true; // 默认开启
          }

          // 为镜像迷宫任务初始化node_scores参数，如果不存在的话
          if (taskName == 'Mirror' &&
              !taskConfigs[taskName].containsKey('node_scores')) {
        ...
        taskTeams[taskName] = List<int>.from(
          configManager.taskConfigs[taskName]!.teams,
        );
      }
    }
  }
  ...
  void _saveConfigToManager() {
    ...
    for (int i = 0; i < _taskKeys.length; i++) {
      ...
      if (configManager.taskConfigs.containsKey(taskName)) {
        ...
        // 对于新添加的两个简单配置项，我们需要特别处理
        if (taskName == 'Daily Lunacy Purchase') {
          configManager.taskConfigs[taskName]!.count =
              taskConfigs[taskName] ?? 0;
        } else if (taskName == 'At Last') {
          configManager.taskConfigs[taskName]!.params['value'] =
              taskConfigs[taskName] ?? 'Do Nothing';
        } else {
          configManager.taskConfigs[taskName]!.params =
              Map<String, dynamic>.from(taskConfigs[taskName] ?? {});
        }
        configManager.taskConfigs[taskName]!.teams = List<int>.from(
          taskTeams[taskName] ?? [],
        );
      } else {
        // 如果任务不存在，创建新的任务配置
  ...
  void _startAllTasks() async {
    ...
    final ok = await CommandGateway().sendTaskCommand(
      context,
      TaskCommand.start,
      (success, msg) {
        if (!success && mounted) {
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      // 对于特殊的两个配置项，不显示设置按钮
                      bool isSpecialConfig =
                          (task == 'Daily Lunacy Purchase' ||
                          task == 'At Last');
                ...
                Consumer<TaskStatusManager>(
                  builder: (context, taskStatusManager, child) {
                    return ElevatedButton(
                      onPressed: !taskStatusManager.isRunning
                          ? _startAllTasks
                          : null,
  ...
  Widget _buildTaskDetailContent(String taskName) {
    ...
    if (taskName == 'E.G.O') {
      return Center(
        child: Text(
          S.of(context).no_other_configs_ego,
          style: const TextStyle(color: Colors.white70, fontSize: 22),
          textAlign: TextAlign.center,
        ),
      );
            ...
            DropdownButton<String>(
              ...
              items: <String>['Enter', 'Skip Battle']
                  .map<DropdownMenuItem<String>>((String value) {
                    String displayText = value == 'Enter'
                        ? S.of(context).luxcavation_enter
                        : value == 'Skip Battle'
                        ? S.of(context).luxcavation_skip_battle
                        : value;
  ...
  Widget _buildMirrorConfigOptions() {
    final TextEditingController? controller =
        textControllers['Mirror_stopPurchaseGiftMoney'];
    ...
    return Column(
      children: [
        ...
        Row(
          ...
          children: [
            Text(
              S.of(context).stop_purchase_gift_money,
              style: const TextStyle(color: Colors.white, fontSize: 16),
            ),
            SizedBox(
              width: 100,
              child: TextField(
                ...
                onChanged: (value) {
                  ...
                  if (value.isNotEmpty) {
                    ...
                    taskConfigs['Mirror']['stopPurchaseGiftMoney'] =
                        parsedValue;
                    // 保存配置
                    _saveConfigToManager();
                  } else {
                ...
                onEditingComplete: () {
                  ...
                  if (controller.text.isEmpty) {
                    ...
                    // 检查当前值是否低于 400
                    int currentValue = int.tryParse(controller.text) ?? 600;
                    if (currentValue < 400) {
                      setState(() {
                        taskConfigs['Mirror']['stopPurchaseGiftMoney'] = 400;
                        controller.text = '400';
                        // 保存配置
                        _saveConfigToManager();
                      });
        ...
        Row(
          ...
          children: [
            Text(
              S.of(context).mirror_difficulty,
              style: const TextStyle(color: Colors.white, fontSize: 16),
            ),
            DropdownButton<String>(
              value: taskConfigs['Mirror']['mirror_mode'] is List
                  ? (taskConfigs['Mirror']['mirror_mode'] as List).first
                        as String
                  : taskConfigs['Mirror']['mirror_mode'] as String?,
              items: <String>['normal', 'hard'].map<DropdownMenuItem<String>>((
                String value,
              ) {
        ...
        Row(
          ...
          children: [
            Text(
              S.of(context).mirror_battle_fail_handle,
              style: const TextStyle(color: Colors.white, fontSize: 16),
            ),
            DropdownButton<String>(
              value:
                  taskConfigs['Mirror']['battle_fail_handle'] ??
                  'continue_next_team',
        ...
        Row(
          ...
          children: [
            Text(
              S.of(context).mirror_shop_fuse_ego_gifts,
              style: const TextStyle(color: Colors.white, fontSize: 16),
            ),
            Switch(
        ...
        Row(
          ...
          children: [
            Text(
              S.of(context).mirror_shop_replace_skill_purchase_ego_gifts,
              style: const TextStyle(color: Colors.white, fontSize: 16),
            ),
            Switch(
              value:
                  taskConfigs['Mirror']['enable_replace_skill_purchase_ego_gifts'] ??
                  true,
              onChanged: (bool value) {
                setState(() {
                  taskConfigs['Mirror']['enable_replace_skill_purchase_ego_gifts'] =
        ...
        Row(
          ...
          children: [
            Text(
              S.of(context).mirror_shop_enhance_ego_gifts,
              style: const TextStyle(color: Colors.white, fontSize: 16),
            ),
            Switch(
        ...
        Row(
          ...
          children: [
            Text(
              S.of(context).mirror_accept_reward,
              style: const TextStyle(color: Colors.white, fontSize: 16),
            ),
            Switch(
        ...
        // 节点权重设置
        Text(
          S.of(context).node_weight_title,
          style: const TextStyle(
            color: Colors.white,
            fontSize: 18,
            fontWeight: FontWeight.bold,
        ...
        Row(
          ...
          children: [
            Text(
              S.of(context).node_event_weight,
              style: const TextStyle(color: Colors.white, fontSize: 16),
            ),
            SizedBox(
              width: 100,
              child: TextField(
                ...
                controller: TextEditingController(
                  text:
                      (taskConfigs['Mirror']['node_scores']?['node_event'] ??
                              20)
                          .toString(),
                ),
                onChanged: (value) {
                  if (value.isNotEmpty) {
                    int parsedValue = int.tryParse(value) ?? 20;
                    taskConfigs['Mirror']['node_scores']['node_event'] =
                        parsedValue;
                    _saveConfigToManager();
                  } else {
                    taskConfigs['Mirror']['node_scores']['node_event'] = 20;
                    _saveConfigToManager();
                  }
                },
              ),
        ...
        Row(
          ...
          children: [
            Text(
              S.of(context).node_regular_encounter_weight,
              style: const TextStyle(color: Colors.white, fontSize: 16),
            ),
            SizedBox(
              width: 100,
              child: TextField(
                ...
                controller: TextEditingController(
                  text:
                      (taskConfigs['Mirror']['node_scores']?['node_regular_encounter'] ??
                              9)
                          .toString(),
                ),
                onChanged: (value) {
                  if (value.isNotEmpty) {
                    int parsedValue = int.tryParse(value) ?? 9;
                    taskConfigs['Mirror']['node_scores']['node_regular_encounter'] =
                        parsedValue;
                    _saveConfigToManager();
                  } else {
        ...
        Row(
          ...
          children: [
            Text(
              S.of(context).node_elite_encounter_weight,
              style: const TextStyle(color: Colors.white, fontSize: 16),
            ),
            SizedBox(
              width: 100,
              child: TextField(
                ...
                controller: TextEditingController(
                  text:
                      (taskConfigs['Mirror']['node_scores']?['node_elite_encounter'] ??
                              1)
                          .toString(),
                ),
                onChanged: (value) {
        ...
        Row(
          ...
          children: [
            Text(
              S.of(context).node_focused_encounter_weight,
              style: const TextStyle(color: Colors.white, fontSize: 16),
            ),
            SizedBox(
              width: 100,
              child: TextField(
                ...
                controller: TextEditingController(
                  text:
                      (taskConfigs['Mirror']['node_scores']?['node_focused_encounter'] ??
                              0)
                          .toString(),
                ),
                onChanged: (value) {
        ...
        Row(
          ...
          children: [
            Text(
              S.of(context).node_abnormality_encounter_weight,
              style: const TextStyle(color: Colors.white, fontSize: 16),
            ),
            SizedBox(
              width: 100,
              child: TextField(
                ...
                controller: TextEditingController(
                  text:
                      (taskConfigs['Mirror']['node_scores']?['node_abnormality_encounter'] ??
                              0)
                          .toString(),
                ),
                onChanged: (value) {
          ...
          children: [
            ...
            SizedBox(
              ...
              child: TextField(
                ...
                controller: TextEditingController(
                  text:
                      (taskConfigs['Mirror']['node_scores']?['node_shop'] ?? 0)
                          .toString(),
                ),
                onChanged: (value) {
                  if (value.isNotEmpty) {
                    int parsedValue = int.tryParse(value) ?? 0;
                    taskConfigs['Mirror']['node_scores']['node_shop'] =
                        parsedValue;
                    _saveConfigToManager();
                  } else {
                    taskConfigs['Mirror']['node_scores']['node_shop'] = 0;
        ...
        Row(
          ...
          children: [
            Text(
              S.of(context).node_boss_encounter_weight,
              style: const TextStyle(color: Colors.white, fontSize: 16),
            ),
            SizedBox(
              width: 100,
              child: TextField(
                ...
                controller: TextEditingController(
                  text:
                      (taskConfigs['Mirror']['node_scores']?['node_boss_encounter'] ??
                              0)
                          .toString(),
                ),
                onChanged: (value) {
  ...
  Widget _buildTaskTeamItem(String taskName, int teamNumber, int index) {
    ...
    // 根据语言本地化流派类型
    String localizedTeamStyle = StyleTypeHelper.getLocalizedStyleType(
      actualTeamStyle,
      context,
    );
      ...
      builder: (BuildContext context, StateSetter setState) {
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final isSelected =
                taskTeams[taskName]?.contains(teamNumber) ?? false;
            ...
            return CheckboxListTile(
              ...
              subtitle: Text(
                '${S.of(context).team_index_label} $teamNumber   ${S.of(context).member_count_label} ${team['selectedMembers']}/12   ${S.of(context).team_style_label} $localizedTeamStyle ',
              ),
              value: isSelected,
              onChanged: (bool? value) {
                setState(() {
  ...
  Widget _buildTaskPreview(String taskName) {
    ...
    if (taskName == 'Mirror') {
      ...
      if (config is Map<String, dynamic>) {
        ...
        // 添加是否开启技能替换&饰品购买选项
        final enableReplaceSkillPurchaseEgoGifts =
            config['enable_replace_skill_purchase_ego_gifts'] ?? true;
        previewWidgets.add(
          Text(
        ...
        // 添加是否开启饰品升级选项
        final enableEnhanceEgoGifts =
            config['enable_enhance_ego_gifts'] ?? true;
        previewWidgets.add(
          Text(
    ...
    if (taskName == "EXP" || taskName == "Thread" || taskName == "Mirror") {
      ...
      if (teams.isNotEmpty) {
        ...
        String teamsText =
            '${S.of(context).selected_teams}: ${teamNames.join(', ')}';
        previewWidgets.add(
          Text(
            teamsText,
            style: const TextStyle(color: Colors.grey, fontSize: 12),
  ...
  Widget _buildInfoContent(String panelType) {
    if (panelType == 'Announcement') {
      ...
      String content =
          latestReleaseInfo ??
          (isLoadingReleaseInfo ? '正在获取最新版本信息...' : '无法获取最新版本信息');
            ...
            child: ElevatedButton(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  Text(
                    S.of(context).check_how_to_use_app,
...
Project repository 项目地址：https://github.com/HSLix/LixAssistantLimbusCompany
                  ...
                  Text(
                    S.of(context).click_to_see_full_declaration,
                    style: const TextStyle(
                      color: Colors.blue,
                      fontSize: 14,
                      decoration: TextDecoration.underline,

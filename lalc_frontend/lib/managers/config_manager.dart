  ...
  void output(OutputEvent event) {
    ...
    try {
      ...
      final File file = File(
        path.join(_logDirectory!.path, 'app_$dateStr.log'),
      );
  ...
  factory TeamConfig.fromJson(Map<String, dynamic> json) {
    return TeamConfig(
      selectedMembers: List<String>.from(
        json['selectedMembers'] as List? ?? [],
      ),
      shopHealAll: json['shopHealAll'] as bool? ?? false,
      selectedTeamStyleType:
          json['selectedTeamStyleType'] as String? ?? 'Bleed',
      selectedPreferEgoGiftTypes: List<String>.from(
        json['selectedPreferEgoGiftTypes'] as List? ?? [],
      ),
      selectedAccessoryTypes: List<String>.from(
        json['selectedAccessoryTypes'] as List? ?? [],
      ),
      giftName2Status: Map<String, String>.from(
        json['giftName2Status'] as Map? ?? {},
      ),
      skillReplacementEnabled: Map<String, bool>.from(
        json['skillReplacementEnabled'] as Map? ?? {},
      ),
      skillReplacementOrders:
          (json['skillReplacementOrders'] as Map?)?.map(
            (key, value) => MapEntry(
              key,
              (value as List)
      ...
      initialEgoGifts: List<int>.from(
        json['initialEgoGifts'] as List? ?? [1, 2, 3],
      ), // 从JSON加载初始饰品配置
      mirrorStarEnabled: Map<String, bool>.from(
        json['mirrorStarEnabled'] as Map? ?? {},
      ), // 从JSON加载镜像星光启用状态
      mirrorStarValues: Map<String, String>.from(
        json['mirrorStarValues'] as Map? ?? {},
      ), // 从JSON加载镜像星光值
    );
  }
...
class ConfigManager {
  ...
  Future<void> init() async {
    try {
      ...
      _themePackConfigFile = File(
        path.join(lalcDirectory.path, 'theme_pack_config.json'),
      );
  ...
  Future<void> _checkAndExtractDefaultConfig() async {
    final luyaoqiConfigDir = Directory(
      path.join(_configsDirectory.path, 'luyaoqi_config'),
    );
    final luyaoqiConfigZip = File(
      path.join(_configsDirectory.path, 'luyaoqi_config.zip'),
    );
    ...
    if (await _taskConfigFile.exists()) {
      try {
        ...
        taskConfigs = jsonMap.map((key, value) {
          return MapEntry(
            key,
            TaskConfig.fromJson(value as Map<String, dynamic>),
          );
        });
        logger.d('任务配置加载成功');
      } catch (e) {
  ...
  void _createDefaultTaskConfig() {
    taskConfigs = {
      'Mail': TaskConfig(enabled: true, count: 1, params: {}, teams: []),
      'E.G.O': TaskConfig(enabled: false, count: 1, params: {}, teams: []),
      'EXP': TaskConfig(
        enabled: true,
        count: 1,
        params: {'luxcavationMode': 'Enter', 'expStage': '09'},
        teams: [],
      ),
      'Thread': TaskConfig(
        enabled: true,
        count: 1,
        params: {'luxcavationMode': 'Enter', 'threadStage': '60'},
        teams: [],
      ),
      'Mirror': TaskConfig(
        enabled: true,
      ...
      'Reward': TaskConfig(enabled: true, count: 1, params: {}, teams: []),
    };
    saveTaskConfig();
  }
  ...
  Future<void> saveTaskConfig() async {
    try {
      final jsonMap = taskConfigs.map(
        (key, value) => MapEntry(key, value.toJson()),
      );
      await _taskConfigFile.writeAsString(
    ...
    if (await _teamConfigFile.exists()) {
      try {
        ...
        teamConfigs = jsonMap.map((key, value) {
          return MapEntry(
            int.parse(key),
            TeamConfig.fromJson(value as Map<String, dynamic>),
          );
        });
        logger.d('队伍配置加载成功');
      } catch (e) {
  ...
  void _createDefaultTeamConfig() {
    ...
    for (int i = 0; i < 20; i++) {
      teamConfigs[i] = TeamConfig(
        ...
        giftName2Status: {},
        skillReplacementEnabled: {for (var name in sinnerNames) name: false},
        skillReplacementOrders: {
  ...
  Future<void> saveTeamConfig() async {
    try {
      final jsonMap = teamConfigs.map(
        (key, value) => MapEntry(key.toString(), value.toJson()),
      );
      await _teamConfigFile.writeAsString(
        JsonEncoder.withIndent('  ').convert(jsonMap),
      );
      logger.d('队伍配置保存成功');
    } catch (e) {
  ...
  Future<void> saveThemePackConfig() async {
    try {
      await _themePackConfigFile.writeAsString(
        JsonEncoder.withIndent('  ').convert(themePackWeights),
      );
      logger.d('主题包配置保存成功');
    } catch (e) {
  ...
  Future<void> saveUserConfig() async {
    try {
      ...
      await _userConfigFile.writeAsString(
        JsonEncoder.withIndent('  ').convert(tempConfig.toJson()),
      );
      logger.d('用户配置保存成功');
    } catch (e) {
      logger.e('保存用户配置失败: $e');
  ...
  void copyTeamConfig(int sourceIndex, int targetIndex) {
    if (teamConfigs.containsKey(sourceIndex)) {
      ...
      final copiedConfig = TeamConfig(
        ...
        selectedPreferEgoGiftTypes: List<String>.from(
          sourceConfig.selectedPreferEgoGiftTypes,
        ),
        selectedAccessoryTypes: List<String>.from(
          sourceConfig.selectedAccessoryTypes,
        ),
        giftName2Status: Map<String, String>.from(sourceConfig.giftName2Status),
        skillReplacementEnabled: Map<String, bool>.from(
          sourceConfig.skillReplacementEnabled,
        ),
        skillReplacementOrders: sourceConfig.skillReplacementOrders.map(
          (key, value) =>
              MapEntry(key, value.map((list) => List<int>.from(list)).toList()),
        ),
        teamName: sourceConfig.teamName,
        initialEgoGifts: List<int>.from(sourceConfig.initialEgoGifts),
        mirrorStarEnabled: Map<String, bool>.from(
          sourceConfig.mirrorStarEnabled,
        ),
        mirrorStarValues: Map<String, String>.from(
          sourceConfig.mirrorStarValues,
        ),
      );
  ...
  Future<void> exportConfigSet(String setName) async {
    try {
      final configSetDir = Directory(
        path.join(_configsDirectory.path, setName),
      );
  ...
  Future<void> saveCurrentConfigAsSet(String setName) async {
    try {
      ...
      final taskConfigFile = File(
        path.join(configSetDir.path, 'task_config.json'),
      );
      final teamConfigFile = File(
        path.join(configSetDir.path, 'team_config.json'),
      );
      final themePackConfigFile = File(
        path.join(configSetDir.path, 'theme_pack_config.json'),
      );
      final userConfigFile = File(
        path.join(configSetDir.path, 'user_config.json'),
      );
      ...
      await themePackConfigFile.writeAsBytes(
        await _themePackConfigFile.readAsBytes(),
      );

      // 用户配置需要特殊处理，清除CDK信息
      final userConfigContent = await _userConfigFile.readAsString();
      final userConfigJson =
          json.decode(userConfigContent) as Map<String, dynamic>;
      userConfigJson['mirrorChanCDK'] = null;
      await userConfigFile.writeAsString(
        JsonEncoder.withIndent('  ').convert(userConfigJson),
      );
  ...
  Future<void> importConfigSet(
    ...
    try {
      ...
      // 检查ZIP文件是否存在
      final configSetZipFile = File(
        path.join(_configsDirectory.path, '$setName.zip'),
      );
      final configSetDirectory = Directory(
        path.join(_configsDirectory.path, setName),
      );
      ...
      if (await configSetZipFile.exists()) {
        ...
        if (importTasks) {
          final sourceTaskConfigFile = File(
            path.join(tempUnzipDir.path, 'task_config.json'),
          );
        ...
        if (importTeams) {
          final sourceTeamConfigFile = File(
            path.join(tempUnzipDir.path, 'team_config.json'),
          );
        ...
        if (importThemePacks) {
          final sourceThemePackConfigFile = File(
            path.join(tempUnzipDir.path, 'theme_pack_config.json'),
          );
        ...
        if (importUserConfig) {
          final sourceUserConfigFile = File(
            path.join(tempUnzipDir.path, 'user_config.json'),
          );
        ...
        if (importTasks) {
          final sourceTaskConfigFile = File(
            path.join(configSetDirectory.path, 'task_config.json'),
          );
          if (await sourceTaskConfigFile.exists()) {
            await _taskConfigFile.writeAsString(
              await sourceTaskConfigFile.readAsString(),
        ...
        if (importTeams) {
          final sourceTeamConfigFile = File(
            path.join(configSetDirectory.path, 'team_config.json'),
          );
          if (await sourceTeamConfigFile.exists()) {
            await _teamConfigFile.writeAsString(
              await sourceTeamConfigFile.readAsString(),
        ...
        if (importThemePacks) {
          final sourceThemePackConfigFile = File(
            path.join(configSetDirectory.path, 'theme_pack_config.json'),
          );
          if (await sourceThemePackConfigFile.exists()) {
            await _themePackConfigFile.writeAsString(
              await sourceThemePackConfigFile.readAsString(),
        ...
        if (importUserConfig) {
          final sourceUserConfigFile = File(
            path.join(configSetDirectory.path, 'user_config.json'),
          );
          if (await sourceUserConfigFile.exists()) {
            await _userConfigFile.writeAsString(
              await sourceUserConfigFile.readAsString(),
  ...
  Future<void> exportConfigToDirectory(String directoryPath) async {
    try {
      ...
      final themePackConfigFile = File(
        '${exportDirectory.path}/theme_pack_config.json',
      );
      ...
      await themePackConfigFile.writeAsString(
        await _themePackConfigFile.readAsString(),
      );
      await userConfigFile.writeAsString(
        JsonEncoder.withIndent('  ').convert(userConfigJson),
      );
